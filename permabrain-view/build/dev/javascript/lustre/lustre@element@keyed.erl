...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(YAG)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(YAG)),
    list(lustre@vdom@vnode:element(YAG))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(YAG)),
    list(lustre@vdom@vnode:element(YAG))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(XZY)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XZY)),
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(XYE)),
    list({binary(), lustre@vdom@vnode:element(XYE)})
) -> lustre@vdom@vnode:element(XYE).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(XYK)),
    list({binary(), lustre@vdom@vnode:element(XYK)})
) -> lustre@vdom@vnode:element(XYK).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(XYQ)})) -> lustre@vdom@vnode:element(XYQ).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(XYU)),
    list({binary(), lustre@vdom@vnode:element(XYU)})
) -> lustre@vdom@vnode:element(XYU).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(XZA)),
    list({binary(), lustre@vdom@vnode:element(XZA)})
) -> lustre@vdom@vnode:element(XZA).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(XZG)),
    list({binary(), lustre@vdom@vnode:element(XZG)})
) -> lustre@vdom@vnode:element(XZG).
...
-spec tbody(
    ...
    list({binary(), lustre@vdom@vnode:element(XZM)})
...
-spec dl(
    list(lustre@vdom@vattr:attribute(XZS)),
    list({binary(), lustre@vdom@vnode:element(XZS)})
) -> lustre@vdom@vnode:element(XZS).
