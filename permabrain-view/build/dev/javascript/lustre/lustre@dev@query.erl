...
-file("src/lustre/dev/query.gleam", 626).
...
text_content(Element, Inline, Content) ->
    case Element of
        ...
        {element, _, _, _, <<"area"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"base"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"col"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"embed"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"hr"/utf8>>, _, _, _, _, _} ->
            Content;
        ...
        {element, _, _, _, <<"link"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"meta"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"param"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"script"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"source"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"style"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"track"/utf8>>, _, _, _, _, _} ->
            Content;

        {element, _, _, _, <<"wbr"/utf8>>, _, _, _, _, _} ->
            Content;
...
-file("src/lustre/dev/query.gleam", 554).
...
matches(Element, Selector) ->
    case {Element, Selector} of
        ...
        {{element, _, _, _, _, _, _, _, _, _}, {has_text, Content}} ->
            text_content(Element, false, <<""/utf8>>) =:= Content;
...
-spec find_matching_in_list(
    list(lustre@vdom@vnode:element(XBG)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XBG), lustre@vdom@path:path()}} |
    {error, nil}.
...
-file("src/lustre/dev/query.gleam", 339).
-spec find_direct_child(
    lustre@vdom@vnode:element(XBB),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XBB), lustre@vdom@path:path()}} |
    {error, nil}.
...
-file("src/lustre/dev/query.gleam", 495).
-spec find_all_matching_in_list(
    list(lustre@vdom@vnode:element(XCO)),
    selector()
) -> list(lustre@vdom@vnode:element(XCO)).
...
-file("src/lustre/dev/query.gleam", 480).
-spec find_all_direct_children(lustre@vdom@vnode:element(XCK), selector()) -> list(lustre@vdom@vnode:element(XCK)).
...
-file("src/lustre/dev/query.gleam", 790).
...
-file("src/lustre/dev/query.gleam", 762).
...
-file("src/lustre/dev/query.gleam", 748).
?DOC(
...
-file("src/lustre/dev/query.gleam", 465).
-spec find_all_in_list(list(lustre@vdom@vnode:element(XCF)), 'query'()) -> list(lustre@vdom@vnode:element(XCF)).
...
-spec find_all(lustre@vdom@vnode:element(XBX), 'query'()) -> list(lustre@vdom@vnode:element(XBX)).
find_all(Root, Query) ->
    case Query of
        {find_element, Selector} ->
...
-spec find_all_in_children(lustre@vdom@vnode:element(XCB), 'query'()) -> list(lustre@vdom@vnode:element(XCB)).
...
-spec find_all_descendants_in_list(
    list(lustre@vdom@vnode:element(XCX)),
    selector()
) -> list(lustre@vdom@vnode:element(XCX)).
...
-spec find_all_descendants(lustre@vdom@vnode:element(XCT), selector()) -> list(lustre@vdom@vnode:element(XCT)).
...
-spec find_in_children(
    lustre@vdom@vnode:element(XAQ),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XAQ), lustre@vdom@path:path()}} |
    {error, nil}.
find_in_children(Element, Query, Index, Path) ->
    case Element of
        {element, _, Key, _, _, _, Children, _, _, _} ->
            find_in_list(
                Children,
                Query,
                begin
                    _pipe = Path,
                    lustre@vdom@path:add(_pipe, Index, Key)
                end,
                0
            );

        {fragment, _, Key, Children, _} ->
            find_in_list(
                Children,
                Query,
                begin
        ...
        {map, _, Key@1, _, Child} ->
            find_path(
                ...
                Query,
                0,
                lustre@vdom@path:subtree(
                    begin
                        _pipe@1 = Path,
                        lustre@vdom@path:add(_pipe@1, Index, Key@1)
                    end
                )
            );

        {memo, _, _, _, View} ->
            find_path(View(), Query, Index, Path);
...
-file("src/lustre/dev/query.gleam", 321).
-spec find_in_list(
    list(lustre@vdom@vnode:element(XAV)),
    'query'(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XAV), lustre@vdom@path:path()}} |
    {error, nil}.
...
?DOC(false).
-spec find_path(
    lustre@vdom@vnode:element(XAL),
    'query'(),
    integer(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XAL), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find(lustre@vdom@vnode:element(XAG), 'query'()) -> {ok,
        lustre@vdom@vnode:element(XAG)} |
    {error, nil}.
find(Root, Query) ->
...
-file("src/lustre/dev/query.gleam", 545).
?DOC(
...
-spec find_descendant_in_list(
    list(lustre@vdom@vnode:element(XBR)),
    selector(),
    lustre@vdom@path:path(),
    integer()
) -> {ok, {lustre@vdom@vnode:element(XBR), lustre@vdom@path:path()}} |
    {error, nil}.
...
-spec find_descendant(
    lustre@vdom@vnode:element(XBM),
    selector(),
    lustre@vdom@path:path()
) -> {ok, {lustre@vdom@vnode:element(XBM), lustre@vdom@path:path()}} |
    {error, nil}.
