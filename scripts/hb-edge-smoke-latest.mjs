...
async function ensureNodeStopped(sb) {
  log('Stopping any running hb daemon...');
  await sandboxExec(sb, 'pkill -f "hb daemon" || true; sleep 1', 30);
}
...
async function updateHyperbeam(sb) {
  log('Updating HyperBEAM to latest origin/edge...');
  await sandboxExec(sb, 'cd /tmp/HyperBEAM && git fetch origin edge', 120);
  const before = await sandboxExec(sb, 'cd /tmp/HyperBEAM && git rev-parse --short HEAD', 30);
  log(`HyperBEAM before update: ${before.result.trim()}`);

  // Discard local modifications and move to latest edge.
  await sandboxExec(sb, 'cd /tmp/HyperBEAM && git reset --hard origin/edge', 60);
  const after = await sandboxExec(sb, 'cd /tmp/HyperBEAM && git rev-parse --short HEAD && git log -1 --format=%cd', 30);
  log(`HyperBEAM after update: ${after.result.trim()}`);

  log('Rebuilding HyperBEAM release...');
  const build = await sandboxExec(sb, 'cd /tmp/HyperBEAM && rebar3 release', 600);
  if (build.exitCode !== 0) {
    throw new Error(`HyperBEAM build failed: exit ${build.exitCode}`);
  }
  log('HyperBEAM build complete.');
}
...
async function main() {
  ...
  await updateHyperbeam(sb);
  await ensureNodeStopped(sb);
