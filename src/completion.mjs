...
export const COMMAND_OPTIONS = {
  ...
  'restart-dev': ['--image', '--port', '--project-dir', '--container-name', '--pull', '--no-pull', '--build-image', '--timeout', '--logs', '--log-lines', '--rm-on-failure', '--dry-run', '--json'],
  'logs-dev': ['--port', '--container-name', '--log-lines', '--follow', '--timestamps', '--since', '--json'],
  'exec-dev': ['--port', '--container-name', '--work-dir', '--env', '--json'],
  'watch-dev': ['--port', '--container-name', '--interval', '--restart', '--timeout', '--json'],
  'wait-dev': ['--port', '--container-name', '--timeout', '--interval', '--json', '--exit-code', '--silent'],
  'check-dev': ['--project-dir', '--image', '--port', '--json', '--exit-code'],
  'verify-dev': ['--port', '--container-name', '--json', '--exit-code'],
  'config-dev': ['--image', '--port', '--project-dir', '--container-name', '--json'],
  'meta-info': ['--url'],
  whois: ['--url'],
  reference: [],
  'transport-status': ['--json'],
  watch: ['--topic', '--kind', '--key', '--interval', '--once'],
  'watch-files': ['--recursive', '--topic', '--kind', '--source-name', '--source-url', '--language', '--visibility', '--for', '--dry-run', '--initial-publish', '--debounce-ms', '--once', '--json'],
  history: ['--no-consensus'],
  diff: ['--format', '--context', '--no-preview', '--local'],
  status: ['--json'],
  health: ['--url', '--json', '--markdown', '--output'],
  grep: ['--regex', '--ignore-case', '--kind', '--topic', '--language', '--key', '--limit', '--context', '--json', '--markdown', '--output'],
  ...
  topics: ['--kind', '--after', '--before', '--sort', '--limit', '--json'],
  config: [],
  ...
  shell: ['--history-path', '--prompt', '--json'],
  client: ['--url'],
  completion: []
};
